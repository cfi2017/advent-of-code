...
impl Puzzle<Vec<Scratchcard>, i32, i32, 2023, 4> for PuzzleDay {
    fn sanitize_input(&self, input: &str) -> Vec<Scratchcard> {
        input.split("\n")
            .filter(|x| !x.is_empty())
    ...
    fn solve_a(&self, input: Vec<Scratchcard>) -> i32 {
        let mut points = 0;
        for card in input {
    ...
    fn solve_b(&self, input: Vec<Scratchcard>) -> i32 {
